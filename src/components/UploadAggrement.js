...
const UploadAggrement = () => {
    const [form] = Form.useForm();
    const dateRef = useRef(null);
    const [uploadDoc, setUploadDoc, isSubmit, setSubmit] = useAggrementStore(
        (state) => [
            state.uploadDoc,
            state.setUploadDoc,
            state.isSubmit,
    ...
    const submitForm = (params) => {
        ...
        getBase64(file).then((data) => {
            ...
            setTimeout(() => {
                window.location.href = "/annotate?type=aggrement";
            }, 1000);
        });
    };
...
export default UploadAggrement;
